...
class Woo_PLSMF_Updater {

	const GITHUB_REPO = 'dataforge/woo-produts-listing-show-more-fields';
	const SLUG        = 'woo-produts-listing-show-more-fields';
	const CACHE_KEY   = 'woo_plsmf_github_release';
	...
	public static function init() {
		...
		add_action( 'admin_post_woo_plsmf_check_updates', array( __CLASS__, 'handle_check_updates' ) );
	}

	public static function check_update( $update, $plugin_data, $plugin_file, $locales ) {
		if ( plugin_basename( WOO_PLSMF_FILE ) !== $plugin_file ) {
			return $update;
		}
		...
		if ( version_compare( WOO_PLSMF_VERSION, $remote_version, '>=' ) ) {
			return $update;
		}
	...
	public static function fix_directory( $result, $options ) {
		...
		if ( ! isset( $options['plugin'] ) || plugin_basename( WOO_PLSMF_FILE ) !== $options['plugin'] ) {
			return $result;
		}
	...
	public static function plugin_info( $result, $action, $args ) {
		...
		$info               = new stdClass();
		$info->name         = 'Woo Products Listing Show More Fields';
		$info->slug         = self::SLUG;
		...
		$info->sections     = array(
			'description' => 'Adds a custom column to the WooCommerce product list table to display various fields.',
	...
	public static function handle_check_updates() {
		...
		check_admin_referer( 'woo_plsmf_check_updates' );
		...
		wp_safe_redirect( add_query_arg( array( 'update_check' => $status ), admin_url( 'admin.php?page=woo-produts-listing-show-more-fields' ) ) );
		exit;
	}
	...
	public static function is_update_available() {
		...
		$remote_version = ltrim( $release->tag_name, 'v' );
		return version_compare( WOO_PLSMF_VERSION, $remote_version, '<' );
	}
